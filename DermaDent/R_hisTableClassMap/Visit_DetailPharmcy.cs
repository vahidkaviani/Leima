//------------------------------------------------------------------------------
// <auto-generated>
//     This code was generated from a template.
//
//     Manual changes to this file may cause unexpected behavior in your application.
//     Manual changes to this file will be overwritten if the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------

namespace DSoftClinicAssistant
{
    using System;
    using System.Collections.Generic;
    
    public partial class Visit_DetailPharmcy
    {
        public int CenterID { get; set; }
        public int ID_Section { get; set; }
        public int ID { get; set; }
        public int RowID { get; set; }
        public int Code { get; set; }
        public string descript { get; set; }
        public Nullable<int> IDSub { get; set; }
        public Nullable<int> idindex { get; set; }
        public Nullable<int> CurrentRow { get; set; }
        public Nullable<bool> Checked { get; set; }
        public string NodePtr { get; set; }
        public Nullable<int> fix { get; set; }
        public Nullable<int> id_User { get; set; }
        public string Date_Edit { get; set; }
        public string Time_Edit { get; set; }
        public Nullable<System.DateTime> Date_Delete { get; set; }
        public Nullable<int> Id_UserDelete { get; set; }
        public Nullable<int> Cancel_Flag { get; set; }
        public string Cancel_FlagDate { get; set; }
        public Nullable<decimal> CodeGoods { get; set; }
        public Nullable<decimal> Quality { get; set; }
        public Nullable<decimal> Route { get; set; }
        public Nullable<decimal> tanavobmasraf { get; set; }
        public Nullable<decimal> days { get; set; }
        public Nullable<decimal> dispose { get; set; }
        public string comment { get; set; }
    }
}
